...
from forms import NewItemForm,IncomeForm,IncomeForm2
...
def income2(request):
    ...
    if request.method == 'GET':
        form = IncomeForm2()
        return render(request,'income2.html',RequestContext(request,{'form':form}))
    else:
        form = IncomeForm(request.POST)
        if form.is_valid():
            name = request.POST.get('name','')
            item = Item(name=name,
                                            num=0)
            item.save()
            return course_render(request,'income2.html')
        else:
            return course_render(request,'income2.html',RequestContext(request,{'form':form,}))
